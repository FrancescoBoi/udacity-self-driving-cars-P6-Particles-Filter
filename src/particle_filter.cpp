...
void ParticleFilter::prediction(double delta_t, double std_pos[],
   ...
   */
   ...
   for (unsigned int ii=0; ii<num_particles; ++ii)
   {
       ...
       double particle_x = p.x + velocity/yaw_rate*(sin(p.theta+yaw_rate*delta_t) - sin(p.theta));
       double particle_y = p.y + velocity/yaw_rate*(cos(p.theta) - cos(p.theta+yaw_rate*delta_t));
       double particle_theta = p.theta + yaw_rate*delta_t;
...
void ParticleFilter::dataAssociation(vector<LandmarkObs> predicted,
  ...
  {
    double temp_dist, min_dist = 1000000.;
   ...
   */
   for(std::size_t ii=0; ii<this->particles.size(); ++ii)
   {
       ...
       for (std::size_t kk=0; kk<map_landmarks.landmark_list.size(); ++kk)
       {
           if (dist(particles.at(ii).x, particles.at(ii).y,
              ...
              < sensor_range)
              {
                  ...
                  temp.id = map_landmarks.landmark_list.at(kk).id_i;
       ...
       for (std::size_t jj=0; jj<observations.size(); ++jj)
       {
         ...
         //this->weights[ii] *= multiv_prob(std_landmark[0], std_landmark[1],
        //    x_map, y_map);
       }
       this->dataAssociation(predicted, temp_obs);
       this->weights[ii] = 1;
       for (std::size_t jj=0; jj<temp_obs.size(); ++jj)
       {
           this->weights[ii] *= multiv_prob(std_landmark[0], std_landmark[1],
               temp_obs.at(jj).x, temp_obs.at(jj).y,
               predicted.at(temp_obs.at(jj).id).x, predicted.at(temp_obs.at(jj).id).y);
       }
